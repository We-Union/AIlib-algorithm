...
def talk_to_chatbot(text):
    ...
    out_sentence = ' '.join(chatbot_words)

    if not out_dict_str:
        return None, out_sentence
    # 将除去pad_token和eos_token的占位符后剩下的字符对应的置信度，保留3位小数输出
    confidence = list(map(lambda x : round(x, 3) , predict_confidence[:len(chatbot_words)]))
    
    output = """
[response]   : {}
[confidence] : {}
    """.format(out_sentence, confidence)
    return None, output

# _, sentence = talk_to_chatbot("hello what's your name ? ")
# print(sentence)